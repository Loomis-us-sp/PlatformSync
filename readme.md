...
Loomis PlatformSync can push data to customer provided end points. Customers can develop these technology agnostic end points as long as they meet the basic minimum common requirements and conform to schema for the expected set of messages. With the Push API, customers are responsible with ensuring the end point is up and running at all times.
...
Security should be implemented as long-lived bearer tokens (or API Keys).
